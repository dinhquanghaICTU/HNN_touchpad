...
// service touchpad HNN_modify - separated into two services
#define TOUCHPAD_NOTIFY_SERVICE_UUID 0xFFE0
#define TOUCHPAD_NOTIFY_UUID 0xFFE1
#define TOUCHPAD_WRITE_SERVICE_UUID 0xFFE3
#define TOUCHPAD_WRITE_UUID 0xFFE4

static const u16 my_touchpadNotifyServiceUUID = 0xFFE0;
static const u16 my_touchpadNotifyCharUUID = 0xFFE1;
static const u16 my_touchpadWriteServiceUUID = 0xFFE3;
static const u16 my_touchpadWriteCharUUID = 0xFFE4;

_attribute_data_retention_ static u8 touchpadNotifyValue[20] = {0};
_attribute_data_retention_  u8 touchpadNotifyCCC[2] = {0, 0};
_attribute_data_retention_ static u8 touchpadWriteValue[20] = {0};
...
static const u8 my_devName[] = {'H', 'N', 'N', 't', 'e', 's', 't'};
...
//// TOUCHPAD NOTIFY attribute values
#if (BLE_OTA_SERVER_ENABLE)
static const u8 my_touchpadNotifyCharVal[5] = {
	CHAR_PROP_READ | CHAR_PROP_NOTIFY,
	U16_LO(TOUCHPAD_NOTIFY_DP_H), U16_HI(TOUCHPAD_NOTIFY_DP_H),
	U16_LO(TOUCHPAD_NOTIFY_UUID), U16_HI(TOUCHPAD_NOTIFY_UUID)};
#else
static const u8 my_touchpadNotifyCharVal[5] = {
	CHAR_PROP_READ | CHAR_PROP_NOTIFY,
	U16_LO(TOUCHPAD_NOTIFY_DP_H), U16_HI(TOUCHPAD_NOTIFY_DP_H),
...
//// TOUCHPAD WRITE attribute values
#if (BLE_OTA_SERVER_ENABLE)
static const u8 my_touchpadWriteCharVal[5] = {
	CHAR_PROP_READ | CHAR_PROP_WRITE | CHAR_PROP_WRITE_WITHOUT_RSP,
	U16_LO(TOUCHPAD_WRITE_DP_H), U16_HI(TOUCHPAD_WRITE_DP_H),
	U16_LO(TOUCHPAD_WRITE_UUID), U16_HI(TOUCHPAD_WRITE_UUID)};
#else
static const u8 my_touchpadWriteCharVal[5] = {
	CHAR_PROP_READ | CHAR_PROP_WRITE | CHAR_PROP_WRITE_WITHOUT_RSP,
	U16_LO(TOUCHPAD_WRITE_DP_H), U16_HI(TOUCHPAD_WRITE_DP_H),
...
static const attribute_t my_Attributes[] = {
	...
	// TOUCHPAD NOTIFY Service
	{4, ATT_PERMISSIONS_READ, 2, 2, (u8 *)(&my_primaryServiceUUID), (u8 *)(&my_touchpadNotifyServiceUUID), 0, 0},
	{0, ATT_PERMISSIONS_READ, 2, sizeof(my_touchpadNotifyCharVal), (u8 *)(&my_characterUUID), (u8 *)(my_touchpadNotifyCharVal), 0, 0},
	{0, ATT_PERMISSIONS_READ, 2, sizeof(touchpadNotifyValue), (u8 *)(&my_touchpadNotifyCharUUID), (u8 *)(touchpadNotifyValue), 0, 0},
	{0, ATT_PERMISSIONS_RDWR, 2, sizeof(touchpadNotifyCCC), (u8 *)(&clientCharacterCfgUUID), (u8 *)(touchpadNotifyCCC), 0, 0},

	// TOUCHPAD WRITE Service
	{3, ATT_PERMISSIONS_READ, 2, 2, (u8 *)(&my_primaryServiceUUID), (u8 *)(&my_touchpadWriteServiceUUID), 0, 0},
	{0, ATT_PERMISSIONS_READ, 2, sizeof(my_touchpadWriteCharVal), (u8 *)(&my_characterUUID), (u8 *)(my_touchpadWriteCharVal), 0, 0},
	{0, ATT_PERMISSIONS_RDWR, 2, sizeof(touchpadWriteValue), (u8 *)(&my_touchpadWriteCharUUID), (u8 *)(touchpadWriteValue), &touchpadwritehandle, NULL},
...
{
#if (BLE_OTA_SERVER_ENABLE)
	tlk_printf("[ATT] OTA service enabled, total attributes: %d\n", ATT_END_H - 1);
	tlk_printf("[ATT] OTA Service UUID: ");
	for(int i = 0; i < 16; i++) {
		tlk_printf("%02x ", my_OtaServiceUUID[i]);
	}
	tlk_printf("\n");
	tlk_printf("[ATT] OTA Characteristic UUID: ");
	for(int i = 0; i < 16; i++) {
		tlk_printf("%02x ", my_OtaUUID[i]);
	}
	tlk_printf("\n");
#else
	tlk_printf("[ATT] OTA service disabled, total attributes: %d\n", ATT_END_H - 1);
#endif
