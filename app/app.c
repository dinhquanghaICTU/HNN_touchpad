...
void update_adv_data_with_runtitle(u8 touchpad1_val, u8 touchpad2_val, u8 touchpad3_val)
{
	...
	u8 manuf_data_len = 1 + 2 + 17;  
	dynamic_advData[pos++] = manuf_data_len;
	dynamic_advData[pos++] = 0xFF;  
	
	dynamic_advData[pos++] = 0x11;  
	dynamic_advData[pos++] = 0x02;  
	
	memcpy(&dynamic_advData[pos], "addruntitle", 11);
	pos += 11;
	
	memcpy(&dynamic_advData[pos], "HNN", 3);
	pos += 3;
		
	dynamic_advData[pos++] = touchpad1_val;
	dynamic_advData[pos++] = touchpad2_val;
...
void task_connect(u8 e, u8 *p, int n)
{
	...
	tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] connect, intA & advA:\r\n", pConnEvt->initA, 12);
...
void task_dle_exchange(u8 e, u8 *p, int n)
{
	...
	tlkapi_send_string_data(APP_CONTR_EVENT_LOG_EN, "[APP][EVT] DLE exchange\r\n", &pEvt->connEffectiveMaxRxOctets, 4);
}
...
int app_host_event_callback(u32 h, u8 *para, int n)
{
	...
	case GAP_EVT_SMP_PAIRING_BEGIN:
	{
		...
		tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] paring begin:\r\n", pEvt, sizeof(gap_smp_pairingBeginEvt_t));
	}
	break;
	...
	case GAP_EVT_SMP_PAIRING_SUCCESS:
	{
		...
		tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] paring success:\r\n", pEvt, sizeof(gap_smp_pairingSuccessEvt_t));
	}
	break;
	...
	case GAP_EVT_SMP_PAIRING_FAIL:
	{
		...
		tlkapi_send_string_data(APP_SMP_LOG_EN, "[APP][SMP] paring fail:\r\n", pEvt, sizeof(gap_smp_pairingFailEvt_t));
	}
	break;
 ...
 */
	...
	tlkapi_send_string_data(APP_LOG_EN, "[APP][INI]Public Address\r\n", mac_public, 6);
...
////////////////// OTA relative ////////////////////////
	...
	#if (UNCLOCK_ADV_RUNTILE)
		runtitle_adv_update_tick = clock_time();  
